...
const rows = 10;  // Update rows for 10x10
const cols = 10;  // Update columns for 10x10
...
// Ensure at least one open space in the last row (either top or bottom)
maze[rows - 1][cols - 2] = 0; // Open space near the top of the last row

// Guarantee a path to the bottom-right corner
maze[rows - 1][cols - 1] = 2; // Exit at the bottom-right corner

// Ensure the top-left and bottom-right positions are valid paths
maze[0][0] = 0;
...
function checkWin() {
    if (maze[player.y][player.x] === 2) {
        window.location.href = '../cong1.html'; // Redirect to a new page on win
    }
}
